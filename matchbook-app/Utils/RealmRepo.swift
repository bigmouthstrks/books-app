...
protocol PrimaryKeyable: AnyObject {
    ...
    dynamic var id: KeyType { get }
    static func primaryKey() -> String?
}

extension PrimaryKeyable {
    static var primaryKeyIdentifier: String { "id" }
}
...
extension Addable {
    static func add(item: Item..., completion: CompletionHandler = nil) {
        ...
        try! realm.write { item.forEach { realm.add($0) } }
    ...
    static func add(itemList: [Item], completion: CompletionHandler = nil) {
        ...
        try! realm.write { realm.add(itemList) }
...
extension Addable where Item: PrimaryKeyable {
    static func add(item: Item..., completion: CompletionHandler = nil) {
        ...
        try! realm.write { item.forEach { realm.add($0, update: .modified) } }
    ...
    static func add(itemList: [Item], completion: CompletionHandler = nil) {
        ...
        try! realm.write { realm.add(itemList, update: .modified) }
...
extension Deletable {
    static func delete(item: Item..., completion: CompletionHandler = nil) {
        ...
        try! realm.write { item.forEach { realm.delete($0) } }
